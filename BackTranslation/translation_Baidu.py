...
class BackTranslation_Baidu(object):
    def __init__(self, appid, secretKey):
        if not appid:
            raise ValueError(
                "'{}': INVALID appid, please register in http://api.fanyi.baidu.com/.".format(
                    appid
                )
            )
        ...
        if not secretKey:
            raise ValueError(
                "'{}': INVALID secretKey, please register in http://api.fanyi.baidu.com/.".format(
                    secretKey
                )
            )
    ...
    def translate(self, text, src="auto", tmp=None):
        ...
        if len(text.encode("utf-8")) > self.MAX_LENGTH:
            ...
            for sentence in original_sentences:
                ...
                back_text.append(
                    self._get_translatedText(self._sendRequest(mid, tmp, src))
                )
    ...
    def _sendRequest(self, text, src, tmp):
        ...
        sign = hashlib.md5(
            (self.appid + text + str(salt) + self.secretKey).encode()
        ).hexdigest()
        url = (
            self.queryURL
            + "?appid="
