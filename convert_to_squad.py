    ...
    with gzip.open(input_path, "rb") as input_file:
        for line in tqdm(input_file, leave=False):
            ...
            questions_allowed_answers = [
                qa.get("answers", []) for qa in mrqa_instance["qas"]
            ]
            ...
            for question_allowed_answers, question_detected_answers in zip(
                ...
                for detected_answer in question_detected_answers:
                    ...
                    question_allowed_answers += [
                        straighten_curly_quotes(answer)
                        for answer in question_allowed_answers
                    ]
                    ...
                    for char_span in detected_answer["char_spans"]:
                        ...
                        matched_answer_text_from_passage = passage[
                            start_char_span : start_char_span
                            + len(detected_answer_text)
                        ]
                        ...
                        if (
                            ...
                            matching_answer_strings.append(
                    ...
                    if matching_answer_strings:
                        ...
                        for answer, start_span in zip(
                            ...
                            for question_allowed_answer in question_allowed_answers:
                                ratio = fuzz.ratio(
                                    question_allowed_answer.lower(), answer.lower()
                                )
                                if ratio > best_ratio:
                                    best_ratio = ratio
                        ...
                        question_squad_format_answers.append(
                            {
                                "answer_start": best_start_char_span,
                                "text": best_answer_string,
                            }
                        )
                    else:
                        print(
            ...
            questions = [
                qa["question"]
                for (qa, ans) in zip(
                    mrqa_instance["qas"], questions_squad_format_answers
                )
                if ans
            ]
            qids = [
            ...
            questions_squad_format_answers = [
                ans for ans in questions_squad_format_answers if ans
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help=("Path to MRQA-format data to convert."),
    )
    parser.add_argument(
        "--output-path",
        type=str,
