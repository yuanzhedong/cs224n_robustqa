...
def synonym_replacement(words, n, answer_words):
    ...
    random_word_list = list(
        set([word for word in words if check_word(word, answer_words)])
    )
...
def swap_word(new_words, answer_words):
    ...
    while (
        random_idx_2 == random_idx_1
        or (new_words[random_idx_2] in answer_words)
        or (new_words[random_idx_1] in answer_words)
    ):
...
def add_word(new_words, answer_words):
    ...
    counter2 = 0
    while (new_words[random_idx] in answer_words) and (
        random_idx > 0 and (new_words[random_idx - 1] in answer_words)
    ):
        random_idx = random.randint(0, len(new_words) - 1)
        counter2 += 1
        if counter >= 10:
