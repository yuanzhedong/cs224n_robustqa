...
def data_augmentation(args, dataset_name, data_dict_collapsed):
    ...
    for idx, answer_dict in enumerate(answer_list):
        ...
        aug_contexts = eda.eda(
            context, answer_words, alpha_sr, alpha_ri, alpha_rs, alpha_rd, num_aug
        )
        for idx_context, aug_context in enumerate(aug_contexts):
            ...
            if len(new_answer_dict["text"]) != 0:
                new_data_dict_collapsed["question"].append(
                    clean_line(question_list[idx])
                )
                ...
                new_data_dict_collapsed["id"].append(
                    str(idx_context) + "eda" + id_list[idx]
                )
