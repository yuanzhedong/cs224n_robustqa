...
def get_dataset(args, tokenizer, split_name, num_aug=0):
    ...
    if args.eda and num_aug > 0:
        datasets_name += (
            "_eda"
            + f"_{num_aug}_{args.alpha_sr}_{args.alpha_ri}_{args.alpha_rs}_{args.alpha_rd}"
        )
    ...
    if (
        ...
        for dataset_path in dataset_paths:
            ...
            if args.back_translate and split_name in [
                "train",
                "finetune",
            ]:  # also apends orignal sentences
                if dataset_name in [
                    "duorc",
                    "race",
                    "relation_extraction",
                ]:  # back translate ood data only
                    dataset_dict_curr = perform_back_translate.perform_back_translate(
                        args, dataset_path, dataset_name
                    )
                    dataset_dict = util.merge(dataset_dict, dataset_dict_curr)

            if args.eda and split_name in [
                "train",
                "finetune",
            ]:  # eda.py does appends original sentences to augmented sentences
                if dataset_name in [
                    "duorc",
                    "race",
                    "relation_extraction",
                ]:  # eda on ood data only
